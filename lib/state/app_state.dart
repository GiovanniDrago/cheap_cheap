...
class AppState extends ChangeNotifier {
  ...
  List<String> galleryAssetSample = [];
  int galleryAssetCount = 0;
  int pendingGalleryRewards = 0;
  Map<String, List<String>> questCompletions = {};
  Map<String, int> dailyExpenseCounts = {};
  Map<String, int> dailyQuestCounts = {};
  List<String> unlockedGallery = [];
  ...
  static Future<AppState> create() async {
    ...
    await state._load();
    await state._loadGalleryAssets();
    return state;
  }
  ...
  Future<void> _load() async {
    ...
    final unlockedJson = _storage.readJsonList('unlockedGallery');
    unlockedGallery = unlockedJson == null
        ? []
        : unlockedJson.map((item) => item as String).toList();
  }
  ...
  Future<void> _loadGalleryAssets() async {
    try {
      ...
      final assets =
          manifest
              .listAssets()
              .where((key) => key.startsWith('assets/gallery/'))
              .where((key) {
      ...
      galleryAssetCount = assets.length;
      galleryAssetSample = assets.take(5).toList();
      _syncUnlockedGallery();
      debugPrint('Gallery assets loaded: $galleryAssetCount');
      if (galleryAssetSample.isNotEmpty) {
        debugPrint('Gallery asset sample: $galleryAssetSample');
      }
      if (pendingGalleryRewards > 0) {
        final pending = pendingGalleryRewards;
        pendingGalleryRewards = 0;
        _unlockGalleryRewards(pending);
      }
    } catch (_) {
      galleryAssets = [];
    }
  ...
  Future<void> refreshGalleryAssets() async {
    await _loadGalleryAssets();
    notifyListeners();
  }

  void _syncUnlockedGallery() {
    if (galleryAssets.isEmpty) {
      return;
    }
    final available = galleryAssets.toSet();
    unlockedGallery = unlockedGallery.where(available.contains).toList();
  }
  ...
  Future<void> _persist() async {
    ...
    await _storage.writeJsonList('unlockedGallery', unlockedGallery);
  }
  ...
  void updateRewardImagesPerQuest(int count) {
    settings = settings.copyWith(rewardImagesPerQuest: count);
    _persist();
    notifyListeners();
  }
  ...
  void _tryCompleteQuest(String questId) {
    ...
    final levelsGained = _addXp(quest.expPoints);
    _unlockGalleryRewards(settings.rewardImagesPerQuest);
    if (levelsGained > 0) {
      _unlockGalleryRewards(levelsGained);
    }
  ...
  void _unlockGalleryRewards(int count) {
    if (count <= 0) {
      return;
    }
    if (galleryAssets.isEmpty) {
      pendingGalleryRewards += count;
      return;
    }
    final random = Random();
    final unlockedSet = unlockedGallery.toSet();
    for (var i = 0; i < count; i++) {
      if (unlockedSet.length < galleryAssets.length) {
        final remaining = galleryAssets
            .where((asset) => !unlockedSet.contains(asset))
            .toList();
        if (remaining.isNotEmpty) {
          final pick = remaining[random.nextInt(remaining.length)];
          unlockedGallery.add(pick);
          unlockedSet.add(pick);
          continue;
        }
      }
      unlockedGallery.add(galleryAssets[random.nextInt(galleryAssets.length)]);
    }
  }
